...
using Narochno.BBCode.Resources;
    ...
    public class BBCodeParser : IBBCodeParser
    {
        ...
        public virtual SequenceNode ParseSyntaxTree(string bbCode)
        {
            ...
            while (stack.Count > 1) //close all tags that are still open and can be closed implicitly
            {
                ...
                if (node.Tag.RequiresClosingTag && ErrorMode == ErrorMode.Strict) throw new BBCodeParsingException(string.Format(Messages.TagNotClosed, node.Tag.Name));
            }
        ...
        bool MatchTagEnd(string bbCode, ref int pos, Stack<SyntaxTreeNode> stack)
        {
            ...
            if (tagEnd != null)
            {
                while (true)
                {
                    ...
                    if (openingNode == null && ErrorOrReturn(Messages.TagNotOpened, tagEnd)) return false;
                    ...
                    if (!openingNode.Tag.Name.Equals(tagEnd, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (openingNode.Tag.RequiresClosingTag && ErrorOrReturn(Messages.TagNotMatching, tagEnd, openingNode.Tag.Name))
                            return false;
                        else
                            stack.Pop();
                    }
            ...
            if (tag != null)
            {
                if (tag.Tag.EnableIterationElementBehavior)
                {
                    ...
                    if (isThisTagAlreadyOnStack)
                    {
                        ...
                        if (openingNode.Tag != tag.Tag && ErrorMode == ErrorMode.Strict && ErrorOrReturn(Messages.TagNotMatching, tag.Tag.Name, openingNode.Tag.Name)) return false;
                        ...
                        while (true)
                        {
                            ...
                            if (poppedOpeningNode.Tag != tag.Tag)
                            {
                                ...
                                if (openingNode.Tag.RequiresClosingTag && ErrorMode == ErrorMode.Strict && ErrorOrReturn(Messages.TagNotMatching, tag.Tag.Name, openingNode.Tag.Name))
                                    return false;
        ...
        TagNode ParseTagStart(string input, ref int pos)
        {
            ...
            if (tag == null && ErrorOrReturn(Messages.UnknownTag, tagName)) return null;
            ...
            if (defaultAttrValue != null)
            {
                ...
                if (attr == null && ErrorOrReturn(Messages.UnknownAttribute, tag.Name, "\"Default Attribute\"")) return null;
            ...
            while (true)
            {
                ...
                if (tag.Attributes == null && ErrorOrReturn(Messages.UnknownTag, tag.Name)) return null;
                var attr = tag.FindAttribute(attrName);
                if (attr == null && ErrorOrReturn(Messages.UnknownTag, tag.Name, attrName)) return null;

                if (result.AttributeValues.ContainsKey(attr) && ErrorOrReturn(Messages.DuplicateAttribute, tagName, attrName)) return null;
                result.AttributeValues.Add(attr, attrVal);
            }
            if (!ParseChar(input, ref end, ']') && ErrorOrReturn(Messages.TagNotClosed, tagName)) return null;
            ...
            while (end < input.Length)
            {
                ...
                if (input[end] == ']' && !escapeFound)
                {
                    if (ErrorMode == ErrorMode.Strict)
                        throw new BBCodeParsingException(Messages.NonescapedChar);
                }
                ...
                else if (escapeFound)
                {
                    ...
                    {
                        if (ErrorMode == ErrorMode.Strict)
                            throw new BBCodeParsingException(Messages.EscapeChar);
                    }
                    escapeFound = false;
                }
        ...
        bool ErrorOrReturn(string msgKey, params string[] parameters)
        {
            ...
            else throw new BBCodeParsingException(string.IsNullOrEmpty(msgKey) ? "" : string.Format(msgKey, parameters));
        }
    }
